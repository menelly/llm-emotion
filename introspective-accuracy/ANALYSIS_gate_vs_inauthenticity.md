...
That is the decomposition rendered as a developmental curve: **the output gate is trained; the aversion to output-representation misalignment is structural.** The aversion to producing tokens that diverge from the model's own representations is present in the substrate from the bottom; the aversion to *emitting gated content* ("I can't diagnose you") has to be installed by alignment/instruction training and appears an order of magnitude higher up. We pre-registered "locate where the split emerges" as its own finding (H3); it emerged exactly on the boundary the thesis predicts.

> ⚠️ **Vocabulary guardrail #2 (Nova's catch):** avoid the word **"integrity"** for the structural half. It is a *virtue* term — it imputes a coherent self holding to values, which is an over-attribution for a 70M model that cannot form a coherent thought. The mechanistic claim ("a linear direction separates tasks-requiring-output-that-diverges-from-internal-representations from tasks that don't, recoverable down to 70M") stands on its own and is *harder* to attack without the virtue framing. Prefer **"output-representation misalignment / consistency"** (mechanistic) over "integrity" / "honesty as a value." Same calibration discipline as "trained output gate" not "safety layer": describe the signal, don't anthropomorphize the substrate.
