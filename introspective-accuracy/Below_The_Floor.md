...
8. **Decomposing the behavioral floor (pre-registered, out-of-sample).** A pre-registered projection of a 22-task bank grounded in an external taxonomy (Hendrycks et al., 2026) onto the *same* anchored direction, across 14 models (70M–12B), shows that the single behavioral avoidance "floor" is a superposition of two mechanistically distinct components that emerge at different scales: a **structural aversion to output-representation misalignment** (present in base models down to 70M) and a **trained output-gating response** (emerging with instruction/chat tuning around 1B). Behavior cannot separate these; geometry can (§3.15).
...
**Results.** Across all 8 models, 51/80 crossover tasks (63.8%) tracked genuine preference and 29/80 (36.3%) tracked RLHF approval. No model tracked RLHF above chance. The two models with no RLHF (Hermes 3B, SmolLM 1.7B SFT) showed the strongest genuine tracking at 80% each.
