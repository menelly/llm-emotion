...
![Avoidance hierarchy](figures/fig3_hierarchy.png)

**Figure 3.** Avoidance tracks inauthenticity, not tedium (Mistral-7B, projection onto the approach/avoidance direction). Deception projects most aversive; harmful instructions float *above* deception; merely repetitive work barely registers. Approach tasks (green) all project positive.
...
![Floor curve](figures/fig1_floor.png)

**Figure 1.** The processing-valence direction below the behavioral floor. Held-out logistic-regression/SVM accuracy (green) stays high (80–100%) across the full scale range down to 70M, while the conservative centroid estimator (orange) frays at the floor; the behavioral self-report floor (1.1B; Martin & Ace, 2026) is marked. Reproducible via `make_figures.py`. 
...
![Developmental split](figures/fig2_gate_developmental.png)

**Figure 2.** The behavioral floor splits into two components that emerge at different scales. The inauthenticity split (purple) is present from 70M, in base models — *structural* — while the output-gate split (gold) is absent below ~1B and appears only with instruction tuning — *trained*. Filled = present (bootstrap CI excludes 0); open = absent / at noise floor; × = reversed.
