...
4. **Joint regression (settles sentiment and perplexity together).** Because perplexity and sentiment may be entangled (§3.11), we fit one model pooled across models (*n*=69 task×model observations, projection and predictors standardized within model): *z*(projection) ~ category + prompt-perplexity + continuation-perplexity + sentiment-projection. The **category coefficient is +1.84 (*t*=16.7, *p*≈5×10⁻²⁵) after partialling out both perplexities and sentiment**, while none of the three candidate confounds is a significant predictor (prompt-ppl *p*=0.11; continuation-ppl *p*=0.18; sentiment-projection *p*=0.74). The approach/avoidance category is the only thing that predicts the projection once the confounds are controlled.
